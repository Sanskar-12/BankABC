...
const [transactions, setTransactions] = useState<Transaction[]>([]);
    const [user,setUser]=useState({accountHolderName:"",accType:"",branchName:"",status:"",balance:0,accId:0})
...
useEffect(() => {
  const fetchDataAccount = async () => {
    try {
      const { data } = await axios.get("http://localhost:8080/api/user/accounts", {
        headers: {
...
useEffect(() => {
  ...
  fetchTransactions();
    ...
    const type = transaction.transactionType === "DEPOSIT" ? "credit" : "debit";
